...
- **Per‑model weekly sub‑limits** (Fable today; Opus / Sonnet before) and **overage** spend, when active
...
- **Sortable columns** (web and macOS app) — click a header to sort by account, plan, session, weekly, model weekly or overage. Usage columns sort lowest first (most room on top), plan and overage largest first; click again to reverse, a third time to return to the headroom order. Accounts with no data for that column stay at the bottom. The choice is remembered
...
- **Each account is weighted by its plan's weekly capacity**, because the bar answers "how much work can the fleet do this week". The 5x / 20x multipliers describe the 5‑hour session bucket; the weekly buckets are closer together — a Max 20x week is about 2× a Max 5x week, and a Max 5x week about 3.5× a Pro week — so the weights are Max 20x = 7, Max 5x = 3.5, Pro = 1. Team seats are labelled `Team 5x` (the profile reports organization type `claude_team` with the limiter's own bucket class, `default_claude_max_5x`) and weighted like that class. Session limits do not change an account's share; they act as throttles that take it out of rotation until the session resets. An unknown plan is weighted as the 5x class and flagged.

  The 2× figure is Craig's observation. A cross‑check from the history log agrees it is nowhere near 4×: within one window, session‑percent burned divided by weekly‑percent burned equals the weekly bucket over the session bucket, and that ratio is about 4.7 on the Max 20x accounts versus about 7.9 on the Team 5x seats. If the session buckets really are 4× apart, that puts the 20x weekly at roughly 2.3× the 5x weekly. The engine re‑reads each account's plan from the profile endpoint once a day, so a plan change shows up without a re‑login.
- **An account's effective usage is its binding window** — `max(session, weekly)`, because that is the one that blocks it. A per‑model weekly limit caps only work on that model, so it counts only for the models the fleet runs on (`FLEET_MODELS` in `src/fleet.mjs`, currently Opus). The fleet runs on Opus 5.5, so Fable's scoped weekly limit is shown per account but does not count toward the fleet. Fleet usage is the weighted mean of effective usage; "free now" is the weighted headroom.
